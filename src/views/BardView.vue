<template>
  ...
  <div class="response" v-if="currentLanguage === 'thai'" >
    ...
    <div v-else>
      ...
      <div class="response-item">
        <span class="response-label">ผลลัพธ์จากBard : </span>
  ...
  methods: {
    addProduct() {
      ...
      const ProductEng = {
       ...
       axios.post('http://127.0.0.1:5000/Bard_product', ProductEng)
        .then(response => {
    ...
    addProductThai() {
      ...
      const ProductThai = {
       ...
       axios.post('http://127.0.0.1:5000/Bard_Thaiproduct', ProductThai)
        .then(response => {
          console.log(response.data.message); 
          this.thairesponse.Thainame = response.data.Thainame;
          this.thairesponse.ThaiBard = response.data.ThaiBard;
