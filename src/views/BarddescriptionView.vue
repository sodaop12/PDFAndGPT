<template>
  ...
  <div class="product-form" v-if="currentLanguage === 'english'" >
    ...
    <form @submit.prevent="addProduct" class="form" >
      ...
      <div class="form-group">
        ...
        <textarea id="description" v-model="productFeature" class="form-textarea" required></textarea>
      </div>
      
      <button type="submit" class="form-button">descrption Product</button>
    </form>
  </div>
    ...
    <div v-else>
      ...
      <div class="response-item">
        <span class="response-label">Bard Result : </span>
        <span class="response-value">{{ response.Bard }}</span>
      </div>
    </div>
  </div>
  ...
  <div class="product-form" v-if="currentLanguage === 'thai'" >
    ...
    <form @submit.prevent="addProductThai" class="form">
      ...
      <button type="submit" class="form-button">ลักษณะสินค้า</button>
    </form>
  </div>
  <div class="response" v-if="currentLanguage === 'thai'" >
    ...
    <div v-else>
      ...
      <div class="response-item">
        ...
        <span class="response-value">{{ thairesponse.ThaiBard }}</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      productFeatureThai: '',
      response: { name: '', Bard: '' },
      thairesponse: { Thainame : '', ThaiBard: '' },
      loading: false
    };
  },
  methods: {
    addProduct() {
      ...
      const ProductEng = {
       ...
       axios.post('http://127.0.0.1:5000/Bard_Desproduct', ProductEng)
        .then(response => {
          console.log(response.data.message); 
          this.response.name = response.data.name;
          this.response.Bard = response.data.Bard;
          this.productName = '';
          this.productPrice = '';
          this.productFeature = '';
          this.loading = false;
    ...
    addProductThai() {
      ...
      const ProductThai = {
        ...
        productFeatureThai: this.productFeatureThai,
        currentLanguage: this.currentLanguage,
      };
       axios.post('http://127.0.0.1:5000/Bard_Desproduct', ProductThai)
        .then(response => {
          console.log(response.data.message); 
          this.thairesponse.Thainame = response.data.name;
          this.thairesponse.ThaiBard = response.data.Bard;
          this.productNameThai = '';
          this.productPriceThai= '';
          this.productFeatureThai = '';
          this.loading = false;
        })
        .catch(error => {
          console.error('Error adding product:', error);
